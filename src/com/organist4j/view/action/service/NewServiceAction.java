...
public class NewServiceAction extends Action implements ISelectionChangedListener {
	...
	public NewServiceAction(ApplicationWindow w) {
		window = w;
		setText("&New Service...@Ctrl+N");
	}
	public void run() {
		...
		NewServiceDialog dlg = new NewServiceDialog(window.getShell(),this);
	...
	public class NewServiceDialog extends TitleAreaDialog implements ModifyListener, SelectionListener {

		NewServiceAction action;
		public NewServiceDialog(Shell shell,NewServiceAction action) {
			super(shell);
			this.action = action;
		}
		...
		protected Control createContents(Composite parent) {
			...
			// Set the message
			setMessage("Choose a name and a date for the service", IMessageProvider.INFORMATION);
		...
		protected Control createDialogArea(Composite parent) {
		    ...
		    if (action.getSelectedService() != null) {
				...
				if (action.getSelectedService().getTemplateName() != null) {
					combo1.select( combo1.indexOf(action.getSelectedService().getTemplateName()) );
